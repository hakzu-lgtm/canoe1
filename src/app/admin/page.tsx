...
export default function AdminPage() {
  const todayReservations = MOCK_RESERVATIONS.filter((r) => r.date === "2026-03-28");
  ...
  const openSessions = MOCK_SESSIONS.filter((s) => s.status === "open" && s.date === "2026-03-28").length;
    ...
    <div>
      ...
      <div className="mb-10 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <StatCard
          ...
          subtitle={`전체 ${MOCK_SESSIONS.filter((s) => s.date === "2026-03-28").length}회 중`}
